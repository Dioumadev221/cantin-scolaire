...
class _AdminHeader extends ConsumerWidget {
  ...
  const _AdminHeader({required this.user, required this.title,
      required this.subtitle, required this.db});
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        PopupMenuButton<String>(
          ...
          onSelected: (v) async {
            if (v == 'profil') _showProfil(context);
            else if (v == 'mdp') _showChangePwd(context);
            else if (v == 'logout') {
          ...
          itemBuilder: (_) => [
            _mi(Icons.person_outline, 'Mon profil', 'profil', const Color(0xFF1a1a1a)),
            _mi(Icons.lock_outline, 'Changer le mot de passe', 'mdp', const Color(0xFF1a1a1a)),
            const PopupMenuDivider(height: 1),
            _mi(Icons.logout, 'Se déconnecter', 'logout', const Color(0xFFEF4444),
          ...
          child: Container(
            ...
            child: Center(child: Text('${user.prenom[0]}${user.nom[0]}',
        ...
        builder: (_, snap) {
          ...
          return Padding(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Text('${d['prenom'] ?? user.prenom} ${d['nom'] ?? user.nom}',
                  style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800)),
              Text(d['email'] ?? user.email,
                  style: const TextStyle(color: Color(0xFF8A8A8A), fontSize: 12)),
              const SizedBox(height: 16),
              _row(ctx, 'Nom', '${d['prenom'] ?? user.prenom} ${d['nom'] ?? user.nom}',
                  () => _edit(ctx, 'Nom', 'nom', d['nom'] ?? user.nom)),
              _row(ctx, 'Téléphone', d['telephone'] ?? 'Ajouter',
                  () => _edit(ctx, 'Téléphone', 'telephone', d['telephone'] ?? '')),
  ...
  Widget _row(BuildContext ctx, String label, String value, VoidCallback onTap) =>
    ...
    showModalBottomSheet(context: ctx, isScrollControlled: true,
      ...
      builder: (c) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Modifier $label', style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w700)),
          const SizedBox(height: 14),
          ...
          SizedBox(width: double.infinity,
            child: ElevatedButton(
              ...
              onPressed: () {
                db.collection('users').doc(user.uid).update({field: ctrl.text.trim()});
                Navigator.pop(c);
              },
      ...
      builder: (ctx) => StatefulBuilder(builder: (c, setS) {
        ...
        return Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 16),
            _pwdF('Mot de passe actuel', c1), const SizedBox(height: 10),
            _pwdF('Nouveau mot de passe', c2), const SizedBox(height: 10),
            _pwdF('Confirmer', c3),
            if (err != null) ...[const SizedBox(height: 10),
              Container(padding: const EdgeInsets.all(10),
                decoration: BoxDecoration(color: const Color(0xFFFEE2E2),
                    borderRadius: BorderRadius.circular(10)),
                child: Text(err!, style: const TextStyle(color: Color(0xFFEF4444), fontSize: 12)))],
            const SizedBox(height: 16),
            SizedBox(width: double.infinity,
              child: ElevatedButton(
                style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1a1a1a),
                ...
                onPressed: loading ? null : () async {
                  if (c2.text != c3.text) { setS(() => err = 'Mots de passe différents'); return; }
                  if (c2.text.length < 6) { setS(() => err = 'Minimum 6 caractères'); return; }
                  setS(() { loading = true; err = null; });
                  try {
                    final u = FirebaseAuth.instance.currentUser!;
                    await u.reauthenticateWithCredential(
                        EmailAuthProvider.credential(email: u.email!, password: c1.text));
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return SingleChildScrollView(
      child: Column(children: [
        _AdminHeader(user: user, title: 'Tableau de bord',
            subtitle: 'Vue d\'ensemble en temps réel', db: db),
        ...
        Padding(
          ...
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snap) {
              ...
              final annulees = todayDocs.where((d) => (d.data() as Map)['statut'] == 'annulee').length;
              final livrees = todayDocs.where((d) => (d.data() as Map)['statut'] == 'recuperee').length;
              final enCours = todayDocs.where((d) =>
                  ['recue','en_cuisine','prete'].contains((d.data() as Map)['statut'])).length;
              ...
              return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                // ── KPIs ──────────────────────────────────────────────────
                Row(children: [
                  _kpi('$total', 'Commandes\naujourd\'hui', '📦', const Color(0xFF1a1a1a)),
                  const SizedBox(width: 10),
                  _kpi('${recettes.toStringAsFixed(0)} F', 'Recettes\ndu jour', '💰',
                      const Color(0xFFFF6B35)),
                ]),
                const SizedBox(height: 10),
                Row(children: [
                  _kpi('$enCours', 'En cours', '🔥', const Color(0xFFF59E0B)),
                  const SizedBox(width: 10),
                  _kpi('$livrees', 'Livrées', '✅', const Color(0xFF22C55E)),
                  const SizedBox(width: 10),
                  _kpi('$annulees', 'Annulées', '❌', const Color(0xFFEF4444)),
                ]),
                const SizedBox(height: 20),

                // ── GRAPHIQUE RECETTES ────────────────────────────────────
                _RevenueChart(recettesParHeure: recettesParHeure, total: recettes),
                const SizedBox(height: 20),

                // ── TOP PLATS ─────────────────────────────────────────────
                const Text('Plats les plus commandés',
                    style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: Color(0xFF1A1A1A))),
                const SizedBox(height: 12),
                if (topPlats.isEmpty)
                  Container(padding: const EdgeInsets.all(20),
                    decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16)),
                    child: const Center(child: Text('Aucune commande aujourd\'hui',
                        style: TextStyle(color: Color(0xFF8A8A8A), fontSize: 13))))
                else
                  ...topPlats.take(5).map((e) => _topRow(e.key, e.value, topPlats.first.value)),

                const SizedBox(height: 20),
                StreamBuilder<QuerySnapshot>(
                  stream: db.collection('users').where('role', isEqualTo: 'etudiant').snapshots(),
                  builder: (_, snap) {
                    final count = snap.data?.docs.length ?? 0;
                    return _kpiWide('$count', 'Étudiants inscrits', '🎓', const Color(0xFF3B82F6));
                  },
                ),
    ...
    child: Container(padding: const EdgeInsets.all(14),
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(label, style: const TextStyle(fontSize: 9, color: Color(0xFF8A8A8A), height: 1.3)),
        ])),
      ]),
    ),
  );

  Widget _kpiWide(String val, String label, String emoji, Color color) => Container(
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ClipRRect(borderRadius: BorderRadius.circular(4),
            child: LinearProgressIndicator(value: pct,
                ...
                color: const Color(0xFFFF6B35), minHeight: 6)),
        ])),
        const SizedBox(width: 12),
        Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
          decoration: BoxDecoration(color: const Color(0xFFFFF3EE),
              borderRadius: BorderRadius.circular(20)),
          child: Text('×$count', style: const TextStyle(
              color: Color(0xFFFF6B35), fontSize: 12, fontWeight: FontWeight.w700))),
      ]),
    );
  }
}
...
class _RevenueChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final maxVal = points.isEmpty ? 1.0 : points.reduce(max).clamp(1.0, double.infinity);
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Évolution des recettes', style: TextStyle(
                fontSize: 14, fontWeight: FontWeight.w800, color: Color(0xFF1A1A1A))),
            Text('Aujourd\'hui · ${total.toStringAsFixed(0)} FCFA au total',
          ...
          Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
            ...
            child: Row(children: [
              ...
              Text('${currentHour}h', style: const TextStyle(
        ...
        Row(
          ...
          children: [7, 9, 11, 13, 15, 17, 19, 21]
              .map((h) => Text('${h}h', style: const TextStyle(
                  color: Color(0xFFCCCCCC), fontSize: 9, fontWeight: FontWeight.w500)))
              .toList(),
        ),
  ...
  Widget _slot(String emoji, String label, double val) => Expanded(
    child: Column(children: [
      ...
      const SizedBox(height: 4),
      Text('${val.toStringAsFixed(0)} F', style: const TextStyle(
          fontSize: 12, fontWeight: FontWeight.w800, color: Color(0xFF1A1A1A))),
      Text(label, style: const TextStyle(fontSize: 9, color: Color(0xFF8A8A8A))),
    ]),
  );

  Widget _slotDivider() => Container(
      width: 0.5, height: 40, color: const Color(0xFFEDEDED));
}
...
class _LineChartPainter extends CustomPainter {
  ...
  _LineChartPainter({
    required this.points, required this.maxVal,
    required this.currentHour, required this.startH, required this.endH,
  });
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 1; i <= 4; i++) {
      ...
      final y = h - (i / 4) * h;
      textPainter.text = TextSpan(text: '${val}F',
    ...
    final fillPaint = Paint()
      ..shader = LinearGradient(
        ...
        colors: [const Color(0xFFFF6B35).withOpacity(0.3), const Color(0xFFFF6B35).withOpacity(0.02)],
    ...
    for (int i = 0; i < offsets.length - 1; i++) {
      final cp1 = Offset((offsets[i].dx + offsets[i + 1].dx) / 2, offsets[i].dy);
      final cp2 = Offset((offsets[i].dx + offsets[i + 1].dx) / 2, offsets[i + 1].dy);
      linePath.cubicTo(cp1.dx, cp1.dy, cp2.dx, cp2.dy, offsets[i + 1].dx, offsets[i + 1].dy);
    }
    ...
    textPainter.text = TextSpan(text: '${curVal}F',
...
class _PlatsTabState extends ConsumerState<_PlatsTab> {
  ...
  Widget build(BuildContext context) => Column(children: [
    _AdminHeader(user: widget.user, title: 'Gestion des plats',
        subtitle: 'Ajoutez et gérez le menu', db: widget.db),
          ...
          return GestureDetector(onTap: () => setState(() => _cat = _cats[i]),
            child: Container(margin: const EdgeInsets.only(right: 8),
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(20),
                border: Border.all(color: active ? const Color(0xFF1a1a1a) : const Color(0xFFEDEDED))),
              child: Text(_cats[i], style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500,
    ...
    Expanded(child: StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snap) {
        ...
        if (_search.isNotEmpty) {
          docs = docs.where((d) =>
              ((d.data() as Map)['nom'] ?? '').toString().toLowerCase()
              .contains(_search)).toList();
        }
        if (docs.isEmpty) return const Center(
            child: Text('Aucun plat', style: TextStyle(color: Color(0xFF8A8A8A))));
        ...
        if (_cat == 'Tous') {
          ...
          for (final doc in docs) {
            final cat = ((doc.data() as Map)['categorie'] ?? '').toString().toLowerCase();
          ...
          for (final cat in order) {
            ...
            items.add(Container(
              ...
              child: Row(children: [
                Text(labels[cat] ?? cat, style: const TextStyle(
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(20)),
                  child: Text('${catDocs.length}', style: const TextStyle(
            ...
            for (final doc in catDocs) {
              items.add(Padding(
                padding: const EdgeInsets.symmetric(horizontal: 16),
                child: _platCard(doc.id, doc.data() as Map<String, dynamic>)));
            }
          }
        ...
        return ListView.builder(
          ...
          itemBuilder: (_, i) => _platCard(docs[i].id, docs[i].data() as Map<String, dynamic>));
      },
    )),
    _addBtn(),
  ]);
  ...
  Widget _platCard(String id, Map<String, dynamic> data) {
    ...
    final isB = (data['categorie'] ?? '').toString().toLowerCase() == 'boissons';
          ...
          child: Row(children: [
            Stack(clipBehavior: Clip.none, children: [
              Container(width: 56, height: 56,
                decoration: BoxDecoration(color: const Color(0xFFFFF3EE),
                    borderRadius: BorderRadius.circular(12)),
                child: Center(child: Text(isB ? _emojiB(data['typeBoisson'] ?? '') : _emojiC(data['categorie'] ?? ''),
                    style: const TextStyle(fontSize: 26)))),
              Positioned(top: -3, right: -3, child: Container(width: 12, height: 12,
                decoration: BoxDecoration(color: dispo ? const Color(0xFF22C55E) : const Color(0xFFEF4444),
                    shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 2)))),
            ]),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(data['nom'] ?? '', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700)),
              Text(isB ? 'Boisson · ${_labelB(data['typeBoisson'] ?? '')}'
                  : '${data['categorie'] ?? ''} · ${data['tempsPreparation'] ?? 0}min',
                  style: const TextStyle(color: Color(0xFF8A8A8A), fontSize: 11)),
              Text('${data['prix'] ?? 0} FCFA', style: const TextStyle(
                  color: Color(0xFFFF6B35), fontSize: 14, fontWeight: FontWeight.w700)),
            ])),
          ])),
        Container(decoration: const BoxDecoration(
            border: Border(top: BorderSide(color: Color(0xFFEDEDED), width: 0.5))),
          child: Row(children: [
            _act('✏️ Modifier', const Color(0xFF1a1a1a), () => _form(id: id, data: data, isB: isB)),
            Container(width: 0.5, height: 36, color: const Color(0xFFEDEDED)),
            _act(dispo ? '● Dispo' : '✕ Indispo',
              dispo ? const Color(0xFF22C55E) : const Color(0xFFEF4444),
              () => widget.db.collection('plats').doc(id).update({'disponible': !dispo})),
            Container(width: 0.5, height: 36, color: const Color(0xFFEDEDED)),
  ...
  Widget _addBtn() => GestureDetector(onTap: _typeSelect,
    child: Container(margin: const EdgeInsets.all(16),
      padding: const EdgeInsets.symmetric(vertical: 14),
      decoration: BoxDecoration(color: const Color(0xFF1a1a1a), borderRadius: BorderRadius.circular(14)),
      child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(Icons.add, color: Colors.white, size: 18), SizedBox(width: 8),
        Text('Ajouter un plat', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w700)),
      ])));
  ...
  void _typeSelect() {
    showModalBottomSheet(context: context, backgroundColor: Colors.white,
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          const Text('Que souhaitez-vous ajouter ?',
              style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800)),
          const SizedBox(height: 20),
          Row(children: [
            Expanded(child: GestureDetector(onTap: () { Navigator.pop(ctx); _form(isB: false); },
              child: _typeCard('🍽️', 'Plat', 'Petit déj, déjeuner, dîner...',
                  const Color(0xFFFFF3EE), const Color(0xFFFFB89A)))),
            const SizedBox(width: 14),
            Expanded(child: GestureDetector(onTap: () { Navigator.pop(ctx); _form(isB: true); },
              child: _typeCard('🥤', 'Boisson', 'Café, thé, jus, eau, lait',
                  const Color(0xFFEFF6FF), const Color(0xFF93C5FD)))),
          ]),
        ]),
      ),
    );
  ...
  Widget _typeCard(String emoji, String label, String sub, Color bg, Color border) =>
      Container(padding: const EdgeInsets.symmetric(vertical: 22),
        decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(18),
            border: Border.all(color: border, width: 1.5)),
        child: Column(children: [
          Text(emoji, style: const TextStyle(fontSize: 40)), const SizedBox(height: 8),
  ...
  void _form({String? id, Map<String, dynamic>? data, required bool isB}) {
    ...
    final temps = TextEditingController(text: data?['tempsPreparation']?.toString() ?? '');
    ...
    showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.white,
      shape: const RoundedRectangleBorder(
          borderRadius: BorderRadius.vertical(top: Radius.circular(24))),
      builder: (ctx) => StatefulBuilder(builder: (c, setS) => SingleChildScrollView(
        padding: EdgeInsets.fromLTRB(20, 20, 20, MediaQuery.of(c).viewInsets.bottom + 20),
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Center(child: Container(width: 40, height: 4,
              decoration: BoxDecoration(color: const Color(0xFFEDEDED),
                  borderRadius: BorderRadius.circular(2)))),
          const SizedBox(height: 14),
          Text(id == null ? (isB ? 'Nouvelle boisson' : 'Nouveau plat') : 'Modifier',
          ...
          if (isB) ...[
            ...
            Wrap(spacing: 8, runSpacing: 8,
              children: _typeBoissons.map((t) {
                ...
                return GestureDetector(onTap: () => setS(() => tb = t['value']!),
                  child: AnimatedContainer(duration: const Duration(milliseconds: 150),
                    padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                    decoration: BoxDecoration(
                      color: sel ? const Color(0xFFEFF6FF) : Colors.white,
                      borderRadius: BorderRadius.circular(12),
                      border: Border.all(color: sel ? const Color(0xFF3B82F6) : const Color(0xFFEDEDED),
                          width: sel ? 2 : 1)),
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      Text(t['label']!, style: TextStyle(fontSize: 12,
                          ...
                          color: sel ? const Color(0xFF1D4ED8) : const Color(0xFF1A1A1A))),
                    ])));
              }).toList()),
            const SizedBox(height: 12),
          ],
          _f('Nom', nom), const SizedBox(height: 10),
          _f('Description', desc), const SizedBox(height: 10),
          ...
          if (!isB) ...[
            ...
            const SizedBox(height: 6),
            DropdownButtonFormField<String>(value: cat,
              decoration: InputDecoration(filled: true, fillColor: const Color(0xFFF5F0EB),
                  border: OutlineInputBorder(borderRadius: BorderRadius.circular(10),
                      borderSide: BorderSide.none)),
              items: const [
                DropdownMenuItem(value: 'petit_dejeuner', child: Text('🌅 Petit déjeuner')),
                DropdownMenuItem(value: 'dejeuner', child: Text('☀️ Déjeuner')),
          ...
          SizedBox(width: double.infinity,
            child: ElevatedButton(
              ...
              onPressed: () {
                final d = isB
                    ? {'nom': nom.text.trim(), 'description': desc.text.trim(),
                        'prix': double.tryParse(prix.text) ?? 0,
                        'categorie': 'boissons', 'typeBoisson': tb,
                        'tempsPreparation': 0, 'disponible': true,
                        'updatedAt': FieldValue.serverTimestamp()}
                    : {'nom': nom.text.trim(), 'description': desc.text.trim(),
                        'prix': double.tryParse(prix.text) ?? 0,
                        'categorie': cat,
                        'tempsPreparation': int.tryParse(temps.text) ?? 0,
                        'disponible': true, 'updatedAt': FieldValue.serverTimestamp()};
                if (id == null) {
                  (d)['createdAt'] = FieldValue.serverTimestamp();
                  widget.db.collection('plats').add(d);
                } else {
                  widget.db.collection('plats').doc(id).update(d);
                }
                Navigator.pop(c);
              },
  ...
  void _del(String id) => showDialog(context: context,
    builder: (_) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(
          ...
          onPressed: () { widget.db.collection('plats').doc(id).delete(); Navigator.pop(context); },
  ...
  Widget _f(String label, TextEditingController ctrl, {bool num = false}) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
        const SizedBox(height: 5),
        TextField(controller: ctrl, keyboardType: num ? TextInputType.number : TextInputType.text,
          decoration: InputDecoration(filled: true, fillColor: const Color(0xFFF5F0EB),
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide.none),
            contentPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12))),
      ]);

  String _emojiC(String c) { switch (c.toLowerCase()) {
    case 'petit_dejeuner': return '🌅'; case 'dejeuner': return '☀️';
    case 'diner': return '🌙'; case 'boissons': return '🥤'; default: return '🍽️'; }}
  String _emojiB(String t) { switch (t) {
    case 'cafe': return '☕'; case 'the': return '🍵'; case 'jus': return '🍊';
    case 'eau': return '💧'; case 'lait': return '🥛'; default: return '🥤'; }}
  String _labelB(String t) { switch (t) {
    case 'cafe': return 'Café'; case 'the': return 'Thé'; case 'jus': return 'Jus';
    case 'eau': return 'Eau'; case 'lait': return 'Lait'; default: return 'Boisson'; }}
}
...
class _MenusTabState extends ConsumerState<_MenusTab> {
  ...
  DateTime get _monday {
    ...
    return now.subtract(Duration(days: now.weekday - 1)).add(Duration(days: _weekOffset * 7));
  }
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      _AdminHeader(user: widget.user, title: 'Gestion des menus',
          subtitle: 'Planifiez les menus de la semaine', db: widget.db),
      _buildWeekNav(),
      _buildDaysRow(),
      Expanded(child: StreamBuilder<QuerySnapshot>(
        stream: widget.db.collection('menus').where('date', isEqualTo: _dateStr).snapshots(),
        builder: (context, snap) {
          ...
          return _buildContent(doc.id, doc.data() as Map<String, dynamic>, _isPast);
        },
      )),
    ]);
  }
  ...
  Widget _buildWeekNav() {
    ...
    final label = '${_monday.day} ${months[_monday.month]} – ${fri.day} ${months[fri.month]} ${fri.year}';
    String sub = _weekOffset == 0 ? 'Cette semaine'
        : _weekOffset == -1 ? 'Semaine passée'
        : _weekOffset == 1 ? 'Semaine prochaine'
        : _weekOffset < 0 ? 'Il y a ${_weekOffset.abs()} sem.' : 'Dans $_weekOffset sem.';

    return Container(color: const Color(0xFFFF6B35),
      padding: const EdgeInsets.fromLTRB(16, 8, 16, 10),
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        _weekBtn('‹', () => setState(() { _weekOffset--; _selectedDay = 0; })),
        GestureDetector(
          onTap: () => setState(() { _weekOffset = 0; _selectedDay = DateTime.now().weekday - 1; }),
          child: Column(children: [
            ...
            Row(children: [
              ...
              if (_weekOffset != 0) ...[const SizedBox(width: 6),
                Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                  decoration: BoxDecoration(color: Colors.white24, borderRadius: BorderRadius.circular(6)),
                  child: const Text('↩ Aujourd\'hui', style: TextStyle(color: Colors.white, fontSize: 9))),
              ],
            ]),
          ]),
        ),
  ...
  Widget _weekBtn(String label, VoidCallback onTap) => GestureDetector(onTap: onTap,
        ...
        return Expanded(child: GestureDetector(
          ...
          child: Container(margin: const EdgeInsets.symmetric(horizontal: 3),
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(12),
              border: Border.all(
                color: active ? const Color(0xFFFF6B35) : isToday ? const Color(0xFFFF6B35) : const Color(0xFFEDEDED),
                width: 1.5)),
            child: Column(children: [
              Text(_days[i], style: TextStyle(fontSize: 9, fontWeight: FontWeight.w600,
                  color: active ? Colors.white70 : isPast ? const Color(0xFFCCCCCC) : const Color(0xFF8A8A8A))),
              const SizedBox(height: 2),
              Text('${day.day}', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700,
                  color: active ? Colors.white : isPast ? const Color(0xFFCCCCCC) : const Color(0xFF1A1A1A))),
            ]))));
      })));
  }
  ...
  Widget _buildEmpty() => Column(children: [
    const Spacer(),
    Text(_isPast ? '🔒' : '📅', style: const TextStyle(fontSize: 56)),
    const SizedBox(height: 12),
    Text(_isPast ? 'Jour passé' : 'Aucun menu pour ce jour',
        style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Color(0xFF1A1A1A))),
    const SizedBox(height: 6),
    Text(_isPast ? 'Impossible de créer un menu passé' : 'Créez le menu de cette journée',
        textAlign: TextAlign.center,
        style: const TextStyle(color: Color(0xFF8A8A8A), fontSize: 13)),
    const Spacer(),
    if (!_isPast)
      GestureDetector(onTap: _createMenu,
        child: Container(margin: const EdgeInsets.all(16),
          padding: const EdgeInsets.symmetric(vertical: 14),
          decoration: BoxDecoration(color: const Color(0xFFFF6B35), borderRadius: BorderRadius.circular(14)),
          child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(Icons.add, color: Colors.white, size: 18), SizedBox(width: 8),
  ...
  Widget _buildContent(String menuId, Map<String, dynamic> data, bool isPast) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(children: [
        ...
        GestureDetector(
          onTap: isPast ? null : () => widget.db.collection('menus').doc(menuId).update({'actif': !actif}),
          child: Container(width: double.infinity,
            padding: const EdgeInsets.symmetric(vertical: 13),
            decoration: BoxDecoration(
              color: actif ? const Color(0xFFD1FAE5) : const Color(0xFFF3F4F6),
              borderRadius: BorderRadius.circular(14),
              border: Border.all(color: actif ? const Color(0xFF22C55E) : const Color(0xFFEDEDED))),
            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Container(width: 8, height: 8,
                  decoration: BoxDecoration(
                    color: actif ? const Color(0xFF22C55E) : const Color(0xFF8A8A8A),
                    shape: BoxShape.circle)),
              const SizedBox(width: 8),
              Text(actif ? 'Menu actif — visible par les étudiants' : 'Menu inactif — invisible',
                  style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600,
                      color: actif ? const Color(0xFF065F46) : const Color(0xFF6B7280))),
              const SizedBox(width: 8),
              Icon(actif ? Icons.visibility : Icons.visibility_off, size: 14,
                  color: actif ? const Color(0xFF065F46) : const Color(0xFF6B7280)),
            ])),
        ),
        if (isPast) ...[
          ...
          Container(
            ...
            child: const Row(children: [
              ...
              SizedBox(width: 8),
              Text('Jour passé — modification impossible',
        ...
        _buildSlot(menuId, '🌅', 'Petit déjeuner', 'petitDej',
            (data['petitDej'] as List?)?.cast<String>() ?? [], isPast, 'petit_dejeuner'),
        const SizedBox(height: 12),
        _buildSlot(menuId, '☀️', 'Déjeuner', 'dejeuner',
            (data['dejeuner'] as List?)?.cast<String>() ?? [], isPast, 'dejeuner'),
        const SizedBox(height: 12),
        _buildSlot(menuId, '🌙', 'Dîner', 'diner',
            (data['diner'] as List?)?.cast<String>() ?? [], isPast, 'diner'),
        const SizedBox(height: 16),
        SizedBox(width: double.infinity,
  ...
  Widget _buildSlot(String menuId, String emoji, String titre, String field,
      List<String> platsIds, bool isPast, String categorie) {
    return Container(
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16),
          border: Border.all(color: const Color(0xFFEDEDED), width: 0.5)),
      child: Column(children: [
        ...
        if (!isPast)
          GestureDetector(onTap: () => _showAddPlat(menuId, field, platsIds, categorie),
            child: Container(margin: const EdgeInsets.all(12),
              padding: const EdgeInsets.symmetric(vertical: 10),
              decoration: BoxDecoration(
                  border: Border.all(color: const Color(0xFFFFB89A), width: 1.5),
                  borderRadius: BorderRadius.circular(10)),
              child: const Center(child: Text('+ Ajouter un plat',
  ...
  Widget _platInSlot(String menuId, String field, String platId, bool isPast) {
    return FutureBuilder<DocumentSnapshot>(
      ...
      builder: (_, snap) {
        ...
        return Container(margin: const EdgeInsets.fromLTRB(12, 0, 12, 8),
          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
          decoration: BoxDecoration(
            color: disponible ? const Color(0xFFF5F0EB) : const Color(0xFFFEF2F2),
            borderRadius: BorderRadius.circular(10)),
          child: Row(children: [
            Text(_emojiCat(data['categorie'] ?? ''), style: const TextStyle(fontSize: 20)),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Expanded(child: Text(data['nom'] ?? '', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600))),
                if (!disponible)
                  Container(
                    margin: const EdgeInsets.only(left: 6),
                    padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                    decoration: BoxDecoration(color: const Color(0xFFEF4444), borderRadius: BorderRadius.circular(6)),
                    child: const Text('Indispo', style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w700))),
              ]),
              Text('${data['prix'] ?? 0} FCFA', style: TextStyle(
                  fontSize: 11, color: disponible ? const Color(0xFFFF6B35) : const Color(0xFFEF4444))),
            ])),
            if (!isPast)
              GestureDetector(
                onTap: () => widget.db.collection('menus').doc(menuId).update({
                  field: FieldValue.arrayRemove([platId])
                }),
                child: const Icon(Icons.close, size: 16, color: Color(0xFF8A8A8A))),
          ]));
      },
    );
  }

  void _showAddPlat(String menuId, String field, List<String> existing,
      String categorie) {
    const labels = {
      'petit_dejeuner': '🌅 Petit déjeuner',
      'dejeuner': '☀️ Déjeuner',
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (_, scrollCtrl) => StreamBuilder<QuerySnapshot>(
          ...
          stream: widget.db.collection('plats')
              .where('disponible', isEqualTo: true)
              .where('categorie', isEqualTo: categorie)
              .snapshots(),
          builder: (_, snap) {
            final allDocs = snap.data?.docs
                .where((d) => !existing.contains(d.id)).toList() ?? [];
              ...
              Padding(
                ...
                child: Row(children: [
                  Expanded(child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Ajouter au menu', style: TextStyle(
                          fontSize: 16, fontWeight: FontWeight.w800)),
                      Text(labels[categorie] ?? categorie,
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                        color: const Color(0xFFFF6B35).withOpacity(0.1),
                        borderRadius: BorderRadius.circular(20)),
                    child: Text('${allDocs.length} plat(s)',
                        style: const TextStyle(
                            color: Color(0xFFFF6B35), fontSize: 11,
                            fontWeight: FontWeight.w700))),
                ])),
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return GestureDetector(
                        onTap: () async {
                          await widget.db.collection('menus')
                              .doc(menuId).update({
  ...
  void _createMenu() {
    widget.db.collection('menus').add({
      'date': _dateStr, 'actif': true,
      'petitDej': [], 'dejeuner': [], 'diner': [],
      'creePar': widget.user.uid,
      'createdAt': FieldValue.serverTimestamp(),
    });
  }

  void _deleteMenu(String id) => showDialog(context: context,
    builder: (_) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(
          style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFEF4444), foregroundColor: Colors.white),
          onPressed: () { widget.db.collection('menus').doc(id).delete(); Navigator.pop(context); },
          child: const Text('Supprimer')),
      ]));

  String _emojiCat(String c) { switch (c.toLowerCase()) {
    case 'petit_dejeuner': return '🌅'; case 'dejeuner': return '☀️';
    case 'diner': return '🌙'; case 'boissons': return '🥤'; default: return '🍽️'; }}
}
...
class _GerantsTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Column(children: [
      _AdminHeader(user: user, title: 'Gérants', subtitle: 'Gérez les comptes gérants', db: db),
      Expanded(child: StreamBuilder<QuerySnapshot>(
        stream: db.collection('users').where('role', isEqualTo: 'gerant_cantine').snapshots(),
        builder: (context, snap) {
          final docs = snap.data?.docs ?? [];
          ...
          return ListView.builder(
            ...
            itemCount: docs.length,
            itemBuilder: (_, i) => _card(context, docs[i].id, docs[i].data() as Map<String, dynamic>));
        },
      )),
      _addBtn(context, ref),
    ]);
  ...
  Widget _card(BuildContext context, String uid, Map<String, dynamic> d) {
    final nom = d['nom'] ?? ''; final prenom = d['prenom'] ?? '';
    final email = d['email'] ?? ''; final cantine = d['nomCantine'] ?? '—';
    return Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14),
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('$prenom $nom', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700)),
          Text(email, style: const TextStyle(fontSize: 11, color: Color(0xFF8A8A8A))),
          Row(children: [
            const Icon(Icons.store_outlined, size: 10, color: Color(0xFFFF6B35)), const SizedBox(width: 3),
            Expanded(child: Text(cantine, style: const TextStyle(
                fontSize: 10, color: Color(0xFFFF6B35), fontWeight: FontWeight.w500),
                overflow: TextOverflow.ellipsis)),
          ]),
        ])),
        GestureDetector(onTap: () => _del(context, uid, '$prenom $nom'),
          child: Container(width: 34, height: 34,
  ...
  void _del(BuildContext ctx, String uid, String nom) => showDialog(context: ctx,
    builder: (_) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(
          style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFEF4444), foregroundColor: Colors.white),
          onPressed: () { db.collection('users').doc(uid).delete(); Navigator.pop(ctx); },
          child: const Text('Supprimer')),
      ]));
  ...
  Widget _addBtn(BuildContext context, WidgetRef ref) => GestureDetector(
    ...
    child: Container(margin: const EdgeInsets.all(16),
      ...
      decoration: BoxDecoration(color: const Color(0xFF1a1a1a), borderRadius: BorderRadius.circular(14)),
      child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(Icons.person_add, color: Colors.white, size: 18), SizedBox(width: 8),
        Text('Créer un gérant', style: TextStyle(
            color: Colors.white, fontSize: 14, fontWeight: FontWeight.w700)),
      ])));

  void _create(BuildContext context, WidgetRef ref) {
    final nom = TextEditingController(), prenom = TextEditingController(),
        email = TextEditingController(), pwd = TextEditingController();
    showDialog(context: context, builder: (_) => AlertDialog(
      title: const Text('Créer un gérant', style: TextStyle(fontWeight: FontWeight.w800)),
      content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
        _tf(nom, 'Nom'), const SizedBox(height: 10),
        _tf(prenom, 'Prénom'), const SizedBox(height: 10),
        _tf(email, 'Email', type: TextInputType.emailAddress), const SizedBox(height: 10),
        ...
        ElevatedButton(
          ...
          onPressed: () async {
            await ref.read(authProvider.notifier).inscription(
                nom.text.trim(), prenom.text.trim(), email.text.trim(), pwd.text.trim(), 'gerant_cantine');
  ...
  Widget _tf(TextEditingController ctrl, String label,
      ...
      TextField(controller: ctrl, keyboardType: type, obscureText: obscure,
        decoration: InputDecoration(labelText: label, filled: true, fillColor: const Color(0xFFF5F0EB),
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide.none)));
}