...
class _PlatsTabState extends ConsumerState<_PlatsTab> {
  String _cat = 'Tous';
  String _search = '';
  final _cats = ['Tous', 'Express', 'Plat du jour', 'Entrées', 'Boissons'];
  ...
  void _form({String? id, Map<String, dynamic>? data, required bool isB}) {
    ...
    String cat = data?['categorie'] ?? 'express';
    String repas = data?['repas'] ?? 'dejeuner';
...
class _MenusTabState extends ConsumerState<_MenusTab> {
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snap) {
              ...
              return _buildContent(doc.id, doc.data() as Map<String, dynamic>);
            },
          ),
        ),
      ],
  ...
  Widget _buildContent(String menuId, Map<String, dynamic> data) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          ...platsIds.map((pid) => _platInSlot(menuId, field, pid)),
          GestureDetector(
            onTap: () => _showAddPlat(menuId, field, platsIds),
            child: Container(
  ...
  Widget _platInSlot(String menuId, String field, String platId) {
  ...
  void _showAddPlat(String menuId, String field, List<String> existing) {
    showModalBottomSheet(
      ...
      builder: (ctx) => StreamBuilder<QuerySnapshot>(
        ...
        builder: (_, snap) {
          final docs =
              snap.data?.docs.where((d) => !existing.contains(d.id)).toList() ??
              [];
          return Padding(
            padding: const EdgeInsets.all(20),
            child: Column(
